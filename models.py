...
class Net_With_Delays(nn.Module):
    ...
    def collect_delays(self):
        ...
        first_delay = delays[0].item().cpu()
        values_per_bins_first_delay = [np.exp(-(delay-first_delay)**2/(self.blocks[0][0][0].SIG.item().cpu() if self.config.DCLSversion == "gauss" else 1e-7)) for delay in range(-(max_delay//2), max_delay//2+1)]
